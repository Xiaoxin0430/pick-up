...
本文档详细描述了随笔系统的API接口，包括用户管理、随笔浏览、收藏和历史记录等功能模块。
...
GET /api/note/categories?skip=0&limit=10
```
...
| pageSize | integer | 否 | 每页显示的随笔数量，最大值为100，默认为10 |

- **请求示例**:

```
GET /api/note/list?categoryId=1
GET /api/note/list?categoryId=1&page=2&pageSize=20
```
...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| noteId | integer | 是 | 随笔ID |

- **请求示例**:

```
GET /api/favorite/check?noteId=1
```
...
```
DELETE /api/favorite/remove?noteId=1
```
