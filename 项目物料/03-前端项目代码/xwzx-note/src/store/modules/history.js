...
export const useHistoryStore = defineStore('history', {
  ...
  actions: {
    ...
    addHistory(note) {
      // 检查是否已存在相同ID的随笔
      const existingIndex = this.history.findIndex(item => item.id === note.id);
    ...
    // 保存到本地存储
    saveHistory() {
      localStorage.setItem('note_history', JSON.stringify(this.history));
    },
